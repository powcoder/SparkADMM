...
import breeze.integrate.DormandPrince54Integrator
import breeze.linalg.{DenseVector, norm}
import org.apache.spark.mllib.regression.LabeledPoint
import org.apache.spark.rdd.RDD
trait ADMMUpdater{
  def xUpdate(data:Array[LabeledPoint],state: ADMMState): ADMMState
  def zUpdate(states: RDD[(Int,ADMMState)],dim:Int): RDD[(Int,ADMMState)]
...
object ADMMUpdater {
  ...
  def seqOp(x:DenseVector[Double],u:DenseVector[Double]):DenseVector[Double]={
    x+u
  }

  def comb(x:DenseVector[Double],u:DenseVector[Double]):DenseVector[Double]={
    x+u
  }

  def linearZUpdate(lambda:Double,rho:Double, dim:Int)(states: RDD[(Int,ADMMState)])={
    ...
    val tmpW = states.map(state=>state._2.x+state._2.u)
    val zeroVector : DenseVector[Double] =  DenseVector.zeros(dim)
    val wBar = tmpW.treeAggregate(zeroVector)(seqOp,comb)
