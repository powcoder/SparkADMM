...
case class SparseLRADMMPrimalUpdater(lambda:Double, rho:Double,
  ...
  override def zUpdate(states:RDD[(Int,ADMMState)], dim:Int): RDD[(Int,ADMMState)] = {
    ADMMUpdater.linearZUpdate(lambda = lambda, rho = rho, dim =dim)(states)
  }
...
class SparseLogisticRegressionWithADMM(numIterations:Int, lambda:Double, rho:Double)
